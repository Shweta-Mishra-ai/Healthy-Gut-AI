...
| Deployment | Render (`render.yaml` / `Procfile`) · Vercel/AWS Lambda (`Mangum`) |
...
2. Create a new **Web Service** on Render, connecting your repository.
3. Render will auto-detect the `render.yaml` blueprint specification.
4. Set environment variables (at minimum `GROQ_API_KEY`) under the Render settings.
5. Render will automatically build the service and deploy it.
